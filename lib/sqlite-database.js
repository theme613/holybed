...
const getAnalytics = () => {
  ...
  // Enhanced symptom analysis with more comprehensive pattern matching
  const topSymptoms = db.prepare(`
    SELECT 
      CASE 
        WHEN LOWER(symptoms) LIKE '%fever%' OR LOWER(symptoms) LIKE '%temperature%' OR LOWER(symptoms) LIKE '%hot%' THEN 'Fever'
        WHEN LOWER(symptoms) LIKE '%headache%' OR LOWER(symptoms) LIKE '%head pain%' OR LOWER(symptoms) LIKE '%migraine%' THEN 'Headache'
        WHEN LOWER(symptoms) LIKE '%chest pain%' OR LOWER(symptoms) LIKE '%chest%' THEN 'Chest Pain'
        WHEN LOWER(symptoms) LIKE '%shortness of breath%' OR LOWER(symptoms) LIKE '%breathing%' OR LOWER(symptoms) LIKE '%breathe%' OR LOWER(symptoms) LIKE '%breath%' THEN 'Shortness of Breath'
        WHEN LOWER(symptoms) LIKE '%stomach%' OR LOWER(symptoms) LIKE '%abdominal%' OR LOWER(symptoms) LIKE '%belly%' OR LOWER(symptoms) LIKE '%tummy%' THEN 'Stomach Pain'
        WHEN LOWER(symptoms) LIKE '%dizziness%' OR LOWER(symptoms) LIKE '%dizzy%' OR LOWER(symptoms) LIKE '%lightheaded%' THEN 'Dizziness'
        WHEN LOWER(symptoms) LIKE '%cough%' OR LOWER(symptoms) LIKE '%coughing%' THEN 'Cough'
        WHEN LOWER(symptoms) LIKE '%sore throat%' OR LOWER(symptoms) LIKE '%throat%' THEN 'Sore Throat'
        WHEN LOWER(symptoms) LIKE '%nausea%' OR LOWER(symptoms) LIKE '%sick%' OR LOWER(symptoms) LIKE '%vomit%' THEN 'Nausea'
        WHEN LOWER(symptoms) LIKE '%fatigue%' OR LOWER(symptoms) LIKE '%tired%' OR LOWER(symptoms) LIKE '%weakness%' OR LOWER(symptoms) LIKE '%weak%' THEN 'Fatigue'
        WHEN LOWER(symptoms) LIKE '%back pain%' OR LOWER(symptoms) LIKE '%back%' THEN 'Back Pain'
        WHEN LOWER(symptoms) LIKE '%joint%' OR LOWER(symptoms) LIKE '%muscle%' OR LOWER(symptoms) LIKE '%ache%' THEN 'Joint/Muscle Pain'
        WHEN LOWER(symptoms) LIKE '%rash%' OR LOWER(symptoms) LIKE '%skin%' OR LOWER(symptoms) LIKE '%itch%' THEN 'Skin Issues'
        WHEN LOWER(symptoms) LIKE '%anxiety%' OR LOWER(symptoms) LIKE '%stress%' OR LOWER(symptoms) LIKE '%panic%' THEN 'Anxiety/Stress'
        WHEN LOWER(symptoms) LIKE '%sleep%' OR LOWER(symptoms) LIKE '%insomnia%' THEN 'Sleep Issues'
        ELSE 'Other Symptoms'
      END as symptom,
      COUNT(*) as count,
      ROUND((COUNT(*) * 100.0 / (SELECT COUNT(*) FROM user_submissions WHERE created_at >= datetime('now', '-30 days'))), 1) as percentage
    FROM user_submissions 
    WHERE created_at >= datetime('now', '-30 days')
    GROUP BY symptom
    HAVING symptom != 'Other Symptoms'
    ORDER BY count DESC
    LIMIT 15
  `).all();
